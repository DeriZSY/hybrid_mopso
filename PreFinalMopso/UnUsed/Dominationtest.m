...
i = 1;
for i = 1: nPoptest
    poptest(i).Velocity=zeros(VarSize);
    %Evaluate Cost
    x = poptest(i).Position;
    poptest(i).Cost=CostFunction(x); 
    
    %Initialize Feasibility 
    poptest(i).CF = fesibJudge(poptest(i)); 
    % Update Personal Best
    poptest(i).Best.Position=poptest(i).Position;
    poptest(i).Best.Cost=poptest(i).Cost;
    poptest(i).Best.CF = fesibJudge(poptest(i)); 
    poptest(i).Best.PF = poptest(i).CF;
end
%% Determine Domination and update the repository

poptest=DetermineDomination(poptest);% set pop.IsDominated

reptest=poptest(~[poptest.IsDominated]); % rep=is not dominated

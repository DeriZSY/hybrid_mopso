function leader=tempOpt(rep,meanCost)
% Find optima solution from a set of solutions  
  ...
  global cEq2;
    ...
    for i = 1: numel(rep)
        flag = 0;
        % rep(i).CF = fesibJudge(rep(i));
        Cost = [rep(i).Cost];
        % rep(i).GridIndex = Cost(1)/meanCost(1) +Cost(2)/meanCost(2) + Cost(3)/meanCost(3) +Cost(4)/meanCost(4);
...
% function fesiableState=fesibJudgeEX(particle)
...
%   particle.sum1 = sum1;
%   particle.sum2 = sum2;
%   if (sum1 > -0.005)&(sum2 > -0.05)&(sum1 < 0)&(sum2 < 0)
%     fesiable2 = true;
%   end
...
%   % fesiable2 = true;
%   fesiableState = fesiableState & fesiable2 & fesiable3;
